...
We have to use these functions alone to gain RCE. Impossible? No. But let's look into how we can do this.
...
* `b"\x83\x01"` - call current function with 1 argument. We will replace with `b"\x83\x00"` to call it with 0 arguments.
* `b"\x17\x00"` - binary add. does first value on stack + second value.

we can use this to generate bytecode to call globals, `b"t\x02\x83\x00"`. Then, we must dynamically construct the word "eval" using only chr via the same methods the put_on_stack function uses. After that, we use the BINARY_SUBSCR opcode, b"\x19\x00", which computes TOP1 = TOP1[TOP2]. In this case, TOP1 will be the output of globals, whilst TOP2 is the string eval, so the result will be the function eval.

Now, loaded on the stack is the function eval. Now, using the same method as put_on_stack again, we construct our python payload that cats the flag. Finally, we wrap it all up with a b"\x83\x01" - call the function eval, loaded onto the stack from globals()['eval'], with one argument.
